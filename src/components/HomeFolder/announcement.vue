            ...
            <div class="flex gap-36">
                <div class="w-2/6">
                    <div class="rounded w-full p-4 bg-gray-200 ">
                        ...
                        <p class="card-title mt-6">{{ lastAnnouncement?.name }}</p>
                        <p class="card-body">
                            {{ lastAnnouncement?.description }}
                        </p>
                    </div>
                    <p class="footer w-full rounded-md p-6 bg-gray-200 text-end"> <span class="font-bold">Created</span>
                        at <span class="date">{{ lastEvent?.created_at
                            }}</span></p>
                </div>
                <div class="w-4/6 grid grid-cols-3 gap-5">
                    <div v-for="otherAnnouncement in otherAnnouncements" :key="otherAnnouncement.id" class="card bg-gray-200">
                    <!-- <div v-for="otherAnnouncement in store.announcementList" :key="otherAnnouncement.id" class="card bg-gray-200"> -->
                        ...
                        <div class="flex flex-col justify-between">
                            <p class="card-title pt-2 pb-1">{{ otherAnnouncement?.name }}</p>
                            <p class="card-body text-justify line-clamp-6 min-h-32 flex-grow">
                                {{ otherAnnouncement?.description }}
                            </p>
                            <p class="footer pt-2 w-full text-end"> <span class="font-bold">Created</span> at <span
                                    class="date">{{ otherAnnouncement?.created_at
                                    }}</span></p>
                        </div>
                    </div>
                    <div class="flex items-center justify-center">
                        <router-link to="/"
                            class="px-5 py-2 bg-gray-200 text-sm text-gray-600 font-bold rounded-3xl hover:text-gray-800 hover:bg-gray-100 border hover:border-2 border-teal-500">See
                            More</router-link>
                    </div>
                </div>
            </div>
