<template>
  <div class="px-5 bg-slate-50 py-5 w-4/5">
    <Breadcrumb/>
    <p class="text-2xl font-bold py-4 text-center border-b-2 mb-2">Imam Package Details</p>
...
const getPackages = async () => {
  ...
  // let customer_package = packages.value
  packageDetails.value = packages.value
  console.log(packageDetails.value, 'line 213');
  selectedDay.value = packageDetails.value.itineraries[0];
  todoList.value = todoList.value.filter(t => t.package_id == packageDetails.value.id)
