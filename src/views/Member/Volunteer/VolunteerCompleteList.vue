<template>
    <div class="px-5 bg-white py-5 rounded-md shadow-md">
      <p class="text-2xl font-bold py-3 border-b">Complete Volunteer List</p>
      <hr />
      ...
      <div v-else class="text-3xl text-center font-semibold pt-2">
        You have not complete any events
      </div>
    </div>
  </template>
  <script setup>
...
const approverdVolunteerList = computed(()=> {
    return store?.volunteerList.filter((item)=>item.volunteer_jobs.status == 3)
})
