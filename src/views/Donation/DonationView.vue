    ...
    <div class="flex item-center bg-teal-50 bg-cover justify-center py-10 px-[5%]">
      <Card>
        <div class="grid grid-cols-5 gap-5 p-8">
          <div v-for="item in accountStore.donationList" @click="donation.donation_method_id = item.id">
            <Label
              for="card"
              class="flex flex-col items-center justify-between rounded-md border-2 border-muted p-4 relative group shadow-lg"
              :class="donation.donation_method_id == item.id ? 'ring-2' : ''"
            >
        ...
        <CardHeader v-if="donation.donation_method_id != ''">
        ...
        <form @submit.prevent="getSession()">
          <CardContent class="grid gap-6">
            ...
            <div class="grid gap-2">
              ...
              <Input id="number" v-model="donation.email" placeholder="" />
            </div>
          </CardContent>
          <CardFooter>
...
const session_token = computed(() => {
  const success_token = route.query?.session_id
...
const getSession = async () => {
  ...
  donation.value.session_token = data.data.clientSecret.id
  console.log(data)
