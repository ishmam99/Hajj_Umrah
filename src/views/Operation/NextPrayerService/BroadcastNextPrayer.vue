<template>
    <div class="px-4 bg-white py-5 w-3/4">
      <div v-if="isEdit == false">
        <div class="flex justify-between items-center pt-4">
          <p class="text-2xl text-cyan-700 font-bold pb-2">Broadcast Prayer Announcement List</p>
        </div>
        <hr />
          ...
          <table class="table-auto w-full">
            <thead>
              <tr class="bg-white text-lg">
                ...
                <th class="p-2 text-start w-1/8">Name</th>
                <th class="p-2 text-start w-2/8">Description</th>
                <th class="p-2 text-center w-1/8">Start Date</th>
                <th class="p-2 text-center w-1/8">End Date</th>
                <th class="p-2 text-center w-1/8">Adhaan </th>
                <th class="p-2 text-center w-1/8">Jamat </th>
                  ...
                  >
                    <DropdownMenu class="w-full">
                      ...
                      <DropdownMenuContent class="w-40">
                        ...
                        >
                        <DropdownMenuItem class="text-yellow-800" @click="edit(event)"
  ...
  const edit = (event) => {
    ...
    // announcemnetStore.value.occurrence_type = event.occurrence_type
    // announcemnetStore.value.human_resource = event.human_resource
    // announcemnetStore.value.material_resource = event.material_resource
    // announcemnetStore.value.description = event.description
    // announcemnetStore.value.address = event.address
    // announcemnetStore.value.state = event.state
    // announcemnetStore.value.start_date = event.start_date
    // announcemnetStore.value.state_time = event.state_time
    // announcemnetStore.value.end_date = event.end_date
    // announcemnetStore.value.end_time = event.end_time
    // announcemnetStore.value.image = event.image
