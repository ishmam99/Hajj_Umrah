  ...
  <div class="px-5 bg-slate-50 py-5">
    ...
    >
      <div class="flex justify-between items-center pb-2">
        <p class="text-2xl text-yellow-600 font-bold">Completed Volunteer Job List</p>
      </div>
    ...
    <div v-if="showDetails == true">
      ...
      <div v-if="volunteerDetails?.applications.length > 0">
        <div
          ...
          :key="item"
          class="p-5 mt-2 rounded-md shadow-lg bg-white"
        >
          <p class="text-2xl mb-3 border-b font-semibold flex justify-between">
            <span>
              Applicant No. {{ index + 1 }} Information
            </span>
            <button @click="volunteerComplete(item?.id)" class="px-3 py-1.5 text-sm bg-teal-600 text-white rounded-md">Job Complete</button>
          </p>
...
const allVolunteerList = computed(() => {
  return store.volunteerWithUser.filter((item) => item.job_status == 'Complete')
})
