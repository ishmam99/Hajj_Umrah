<template>
  <div class="px-5 bg-slate-50 py-5 w-3/4">
    <div class="flex justify-between items-center py-3">
      ...
      <button @click="$router.go(-1)" class="btn btn-primary text-white flex items-center gap-2"><svg
    ...
    <div class="bg-slate-50 py-1">
      ...
      <div class="flex w-full justify-between mt-5 border-t-2 border-x-2 border-teal-600">
        <button @click="currentTab = 'date'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'date' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Date
        </button>
        <button @click="currentTab = 'imam'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'imam' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Imam
        </button>
        <button @click="currentTab = 'agent'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'agent' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Agent
        </button>
        <button @click="currentTab = 'flight'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'flight' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Flight
        </button>
        <button @click="currentTab = 'bus'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'bus' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Bus
        </button>
        <button @click="currentTab = 'hotel'" class="py-3 px-5 border-e border-white font-semibold w-full text-xl"
          :class="currentTab == 'hotel' ? 'bg-white text-teal-700' : 'bg-teal-600 text-white'">
          Hotel
        </button>
        <button @click="currentTab = 'fr'" class="py-3 px-5 border-e border-white w-full font-semibold text-xl"
      ...
      <div v-if="currentTab == 'date'" class="bg-white border-x-2 border-teal-700 border-b-2">
        ...
        <div class="px-10">
          <div class="flex items-center my-5 justify-center" v-if="packageDetails">
            ...
            <input v-model="packageDetails.start_at" type="date" class="input input-bordered w-full" />
          </div>
          <div class="flex items-center my-5 justify-center" v-if="packageDetails">
            <h1 class="w-full px-5">End Date :</h1>
            <input v-model="packageDetails.end_at" type="date" class="input input-bordered w-full" />
          </div>

          <div class="flex justify-end items-center py-2">
            <button @click="updatePackageData()" class="btn btn-success text-white" :disabled="updating">
              <svg xmlns="http://www.w3.org/2000/svg" v-if="updating" class="animate-spin" viewBox="0 0 24 24"
        ...
        <div class="border-t-2 border-teal-600 p-5">
          <div class="flex justify-start gap-5 items-center w-full py-2 px-5" v-if="packageDetails">
            ...
            <select v-model="packageDetails.package_status.date_approve" class="select select-bordered w-1/2">
      ...
      <div v-if="currentTab == 'imam'" class="bg-white border-x-2 border-teal-700 border-b-2">
        ...
        <div class="border-t-2 border-teal-600 p-5">
          <div class="flex justify-start gap-5 items-center w-full py-2 px-5" v-if="packageDetails">
            ...
            <select v-model="packageDetails.package_status.imam_approve" class="select select-bordered w-1/2">
              <option disabled selected>Select Status</option>
              <option value="0">1. In Preparation</option>
              <option value="1">1a. In Preparation -Started</option>
              <option value="1">1b. In Preparation -In Work</option>
      ...
      <div v-if="currentTab == 'agent'" class="bg-white border-x-2 border-teal-700 border-b-2">
        ...
        <div class="border-t-2 border-teal-600 p-5">
          <div class="flex justify-start gap-5 items-center w-full py-2 px-5" v-if="packageDetails">
            ...
            <select v-model="packageDetails.package_status.agent_approve" class="select select-bordered w-1/2">
              <option disabled selected>Select Status</option>
              <option value="0">1. In Preparation</option>
              <option value="1">1a. In Preparation -Started</option>
              <option value="1">1b. In Preparation -In Work</option>
        ...
        <div class="px-10">
          ...
          <div class="border-t-2 border-teal-600 p-5">
            <div class="flex justify-start gap-5 items-center w-full py-2 px-5" v-if="packageDetails">
              ...
              <select v-model="packageDetails.package_status.bus_approve" class="select select-bordered w-1/2">
                <option disabled selected>Select Status</option>
                <option value="0">1. In Preparation</option>
                <option value="1">1a. In Preparation -Started</option>
                <option value="1">1b. In Preparation -In Work</option>
      ...
      <div v-if="currentTab == 'fr'" class="bg-white border-x-2 border-teal-700 border-b-2">
        ...
        <div class="p-5">
            ...
            <table class="table-auto w-full  shadow-md rounded-lg border-collapse border border-gray-300 ">
              ...
              <tbody>
                <template v-for="(itinerary, index) in packageDetails.itineraries" :key="itinerary.id">
                  <tr class="bg-gray-100 hover:bg-white transition duration-200"
                    v-for="activity in itinerary.activities" :key="activity.id">
          ...
          <div
            class="border-t-4 border-teal-600 p-5 mt-5 bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg shadow-lg">
            <div class="flex justify-start gap-5 items-center w-full py-2 px-5">
              ...
              <button @click="updatePackageStatus"
                class="btn btn-info text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-600 transition duration-300"
                :disabled="applying">
                <svg xmlns="http://www.w3.org/2000/svg" v-if="applying" class="animate-spin mr-2" viewBox="0 0 24 24"
                  width="18" height="18" fill="currentColor">
                  <path
                    d="M18.364 5.63604L16.9497 7.05025C15.683 5.7835 13.933 5 12 5C8.13401 5 5 8.13401 5 12C5 15.866 8.13401 19 12 19C15.866 19 19 15.866 19 12H21C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C14.4853 3 16.7353 4.00736 18.364 5.63604Z">
                  </path>
