<template>
  <DefaultLayout>
    <div
      class="px-20 pt-20 mt-5 py-5 bg-teal-800 bg-[url('/src/assets/image/home/bg123.png')] bg-repeat shadow-xl"
    >
      ...
      <div class="grid grid-cols-3 gap-3 w-full">
        <div v-for="surah in surahList.data" v-if="surahList">
          <Card class="rounded-2xl">
            <CardHeader>
              <router-link
                :to="{ name: 'explore/quranview', params: { id: surah.number } }"
                class="text-3xl text-slate-700"
                >{{ surah.englishName }}
                <span class="text-xl">({{ surah.name }})</span></router-link
              >
              <h1 class="font-semibold text-xl text-amber-600">
