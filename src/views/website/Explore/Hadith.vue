  ...
  <DefaultLayout>
    ...
    <div class="p-20 bg-gray-100 gap-5">
          ...
          <Card class="rounded-2xl" :class="hadith.id == currentHadith?.id ? 'bg-green-600' : ''">
            <CardHeader @click="getHadithSection(hadith)">
              <CardTitle
                ...
                :class="hadith.id == currentHadith?.id ? 'text-white' : ''"
                >{{ hadith.name }}
                <span class="text-xl">({{ hadith.arabic_book_name }})</span></CardTitle
              >
            </CardHeader>
            <CardContent
              @click="getHadithSection(hadith)"
              class="text-gray-600"
              :class="hadith.id == currentHadith?.id ? 'text-white' : ''"
            >
      ...
      <Card class="py-10 mt-5" v-if="currentHadith">
        <CardHeader>
          <CardTitle class="text-4xl text-center flex justify-center items-center gap-1"
            >{{ currentHadith.name }} ({{ currentHadith.arabic_book_name }})
          </CardTitle>
          <CardTitle
            v-if="currentSection"
            class="text-2xl text-center flex justify-center items-center gap-1"
            >Section : {{ currentSection.name }}
          </CardTitle>
          <CardTitle
            v-if="hadith"
            class="text-xl text-center flex justify-center items-center gap-1"
            >Chapter : {{ hadith.name }}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div class="grid grid-cols-4 gap-5">
            <div v-for="section in currentHadith?.data">
              <Button variant="secondary" @click="getHadithChapters(section)" class="w-full">{{
                section.name
              }}</Button>
            </div>
          </div>
...
const getHadithSection = async (hadith) => {
  axios
    .get(`https://hadithadmin.islamicdigitallane.com/api/v1/hadithSection/${hadith.id}`)
    .then((response) => {
...
const getHadithChapters = async (section) => {
  axios
    .get(`https://hadithadmin.islamicdigitallane.com/api/v1/hadithChapter/${section.id}`)
    .then((response) => {
...
const getHadith = async (chapter) => {
  axios
    .get(`https://hadithadmin.islamicdigitallane.com/api/v1/hadith/${chapter.id}`)
    .then((response) => {
